...
Is your answer the number `2175919200`?  Great work! Your computer works as expected. One of the nice things about writing math expressions in JavaScript is that what you write looks a lot like what  you would punch into a calculator, or even write out by hand on paper ... who does that anymore?
...
The **remainder** symbol `%`, also known as the **modulus** operator, calculates the remainder value of the number on the left divided by the number on the right. The application of the **remainder** operator on two numbers will always produce a number of the same sign as the number on the left, which is the amount left over after evenly dividing the number on the left by the number on the right.
...
In most programming languages there are multiple types of numbers, for example there are **Integers** which are whole numbers, or numbers without a fractional component, and there are **Floating Point Numbers** which are numbers which are represented by some fraction or fractional approximation. Many people think of **Floating Point Numbers** also called **Floats** as numbers that have some number of digits after the decimal point or `.`, such as `3.14159` or `2.71828`. These are numbers which can be approximated in JavaScript by taking some integer, and raising it to some exponential value.
...
Why do these kind numbers exist? Well it's a bit complicated, but is has to do with how much information can be stored in a regular old JavaScript **Number** type. To make a long story short, there is only so much room within the default **Number** type, and so as your number gets **really** large, as in quadrillions, then it's possible that when you combine that number with another using math you *could* get the wrong answer. By using **BigInt** for **Integer** numbers you do not have to worry about those limitations, but you **DO** have to use them together with other **BigInt** numbers, and you **DO** have to be using a modern browser or other JavaScript environment.
